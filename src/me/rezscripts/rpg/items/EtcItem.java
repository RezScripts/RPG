...
import me.rezscipts.rpg.players.HealType;
import me.rezscipts.rpg.warps.WarpLocation;
import me.rezscipts.rpgexperience.utils.RFormatter;
...
public class EtcItem extends RPGItem {
     ...
     * have their listeners within this method. For rezscipts, basically every item that has a
    ...
    public static void postInitialize() {
        ...
        ItemManager.registerItemRunnable("old_player", new ItemRunnable() {
            ...
            public void run(Event e, Player p, PlayerDataRPG pd) {
                ...
                p.sendMessage(ChatColor.GOLD + p.getName() + " is a true RPGExperience veteran!");
                for (Entity p2 : p.getNearbyEntities(10, 10, 10))
                    if (p2 instanceof Player)
                        ((Player) p2).sendMessage(ChatColor.GOLD + p.getName() + " is a true RPGExperience veteran!");
            }
        });
        ...
        ItemManager.registerItemRunnable("july_2014_voter", new ItemRunnable() {
            ...
            public void run(Event event, final Player p, PlayerDataRPG pd) {
                ...
                p.sendMessage(ChatColor.GOLD + "[Username] - winner of the [Date] voting contest!");
                for (Entity p2 : p.getNearbyEntities(10, 10, 10))
                    if (p2 instanceof Player)
                        ((Player) p2).sendMessage(ChatColor.GOLD + "Congratulations to [Username], winner of the [Date] voting contest!");
            }
        });
