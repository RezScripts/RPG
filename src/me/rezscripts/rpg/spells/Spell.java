...
import me.rezscipts.rpg.commands.owner.ManaCommand;
import me.rezscipts.rpg.mobs.MobData;
import me.rezscipts.rpg.mobs.MobManager;
import me.rezscipts.rpg.parties.PartyManager;
...
public class Spell {
    ...
    public static void notify(Player p, String s) {
        if (plugin.getPD(p) != null && plugin.getPD(p).getOption(RPGOption.SPELL_MESSAGES))
    ...
    public void cast(Player p, PlayerDataRPG pd) {
        ...
        if (item == null || !SpellManager.validateWeapon(pd, pd.classType, item)) {
            ...
            if (pd.mana < manaCost) {
                ...
                if (res) {
                    if (pd.getOption(RPGOption.MANA_MESSAGES))
